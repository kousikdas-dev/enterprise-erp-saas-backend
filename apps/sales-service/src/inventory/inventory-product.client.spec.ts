...
function config(): ConfigService<SalesEnvironmentVariables, true> {
  return {
    get: (key: string) => {
      if (key === 'INVENTORY_SERVICE_URL') return INVENTORY_SERVICE_URL;
...
describe('InventoryProductClient', () => {
  const actor = {
  ...
  const uomOptions: UomOptionsResponse = {
    productId,
    base: { unitOfMeasureId: 'unit-ea', code: 'EA', name: 'Each' },
    alternatives: [
      {
  ...
  function createClient(get: jest.Mock) {
    return new InventoryProductClient(
  ...
  it('resolves to the unwrapped payload from the response envelope', async () => {
    ...
    const result = await client.getUomOptions(actor, productId);

    expect(result).toEqual(uomOptions);
  });

  it('calls the correct URL with the correct internal headers', async () => {
    const get = jest.fn().mockReturnValue(
      of({
        data: { success: true, statusCode: 200, data: uomOptions, timestamp: '' },
      }),
    );
    const client = createClient(get);

    await client.getUomOptions(actor, productId);

    expect(get).toHaveBeenCalledWith(
      `http://localhost:3002/api/v1/internal/products/${productId}/uom-options`,
      {
        headers: {
  ...
  it('throws ServiceUnavailableException when Inventory is unreachable', async () => {
    const get = jest.fn().mockReturnValue(throwError(() => axiosError()));
    const client = createClient(get);

    await expect(
      client.getUomOptions(actor, productId),
