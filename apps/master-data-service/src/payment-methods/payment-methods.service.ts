import {
  BadRequestException,
  ConflictException,
  Injectable,
  NotFoundException,
} from '@nestjs/common';
import { IdentityAuditClient } from '../audit/identity-audit.client';
import { ActorContext, RequestAuditMeta } from '../auth/actor-context';
import { PrismaService } from '../prisma/prisma.service';
import { isUniqueConstraintError } from '../prisma/prisma-errors';
import { toPaymentMethod } from './dto/payment-method-response';
import {
  CreatePaymentMethodDto,
  UpdatePaymentMethodDto,
} from './dto/payment-method.dto';

@Injectable()
export class PaymentMethodsService {
  constructor(
    private readonly prisma: PrismaService,
    private readonly audit: IdentityAuditClient,
  ) {}

  async create(
    actor: ActorContext,
    dto: CreatePaymentMethodDto,
    request?: RequestAuditMeta,
  ) {
    try {
      const row = await this.prisma.paymentMethod.create({
        data: {
          tenantId: actor.tenantId,
          code: dto.code.trim().toUpperCase(),
          name: dto.name.trim(),
        },
      });

      await this.audit.record({
        actor,
        action: 'payment_method.created',
        resource: 'payment_method',
        resourceId: row.id,
        metadata: {
          code: row.code,
          name: row.name,
        },
        request,
      });

      return toPaymentMethod(row);
    } catch (error) {
      if (isUniqueConstraintError(error)) {
        throw new ConflictException(
          'Payment method code already exists in this tenant',
        );
      }

      throw error;
    }
  }

  async list(actor: ActorContext) {
    const rows = await this.prisma.paymentMethod.findMany({
      where: {
        tenantId: actor.tenantId,
      },
      orderBy: {
        code: 'asc',
      },
    });

    return {
      items: rows.map(toPaymentMethod),
    };
  }

  async getById(actor: ActorContext, id: string) {
    return toPaymentMethod(await this.require(actor, id));
  }

  async update(
    actor: ActorContext,
    id: string,
    dto: UpdatePaymentMethodDto,
    request?: RequestAuditMeta,
  ) {
    await this.require(actor, id);

    const data: {
      code?: string;
      name?: string;
      isActive?: boolean;
    } = {};

    if (dto.code !== undefined) {
      data.code = dto.code.trim().toUpperCase();
    }

    if (dto.name !== undefined) {
      data.name = dto.name.trim();
    }

    if (dto.isActive !== undefined) {
      data.isActive = dto.isActive;
    }

    if (Object.keys(data).length === 0) {
      throw new BadRequestException('No fields to update');
    }

    try {
      const row = await this.prisma.paymentMethod.update({
        where: {
          id,
        },
        data,
      });

      await this.audit.record({
        actor,
        action: 'payment_method.updated',
        resource: 'payment_method',
        resourceId: row.id,
        metadata: data,
        request,
      });

      return toPaymentMethod(row);
    } catch (error) {
      if (isUniqueConstraintError(error)) {
        throw new ConflictException(
          'Payment method code already exists in this tenant',
        );
      }

      throw error;
    }
  }

  async require(actor: ActorContext, id: string) {
    const row = await this.prisma.paymentMethod.findFirst({
      where: {
        id,
        tenantId: actor.tenantId,
      },
    });

    if (!row) {
      throw new NotFoundException('Payment method not found');
    }

    return row;
  }
}