...
/**
 * Phase C1/C2 (Purchase Accounting Integration) — sibling to
 * AccountingTaxCodeClient, same direct-HTTP-to-accounting-service pattern
 * (bypasses the gateway, internal-secret + actor headers). Never called
 * from inside a Prisma transaction (mirrors the repo-wide rule already
 * followed for every other external call from this service) — always
 * invoked after the caller's own transaction has committed.
 */
@Injectable()
