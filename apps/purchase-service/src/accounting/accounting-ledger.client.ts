...
// AccountingJournalClient's /internal/* routes. Confirmed by reading
// accounting-service's actor.guard.ts directly.
...
export interface AccountsPayableMapping {
  accountId: string;
  accountCode: string;
  accountName: string;
}
...
export class AccountingLedgerClient {
  ...
  /** Resolves the tenant's ACCOUNTS_PAYABLE account mapping, or null if the
   * tenant has not configured one yet — a data-completeness gap, not an
   * error, so callers surface it as such rather than failing the request. */
  async findAccountsPayableMapping(
    actor: ActorContext,
  ): Promise<AccountsPayableMapping | null> {
    ...
    try {
      ...
      const row = rows.find(
        (item) => item.purpose === 'ACCOUNTS_PAYABLE' && item.externalRefId === '',
      );
      if (!row || !row.account) {
        return null;
      }
