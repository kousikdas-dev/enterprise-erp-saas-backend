...
## 24. Supplier Payment Reversal & Retry Lifecycle — Phase 3.8 (Implemented)

> **Status: IMPLEMENTED 2026-09-25** (commit `4acc47f289ab56da4b93833aa415c71ed5d94b23`, `feat(purchase): add supplier payment reversal and retry lifecycle`). This section supersedes §22.9's closing paragraph ("Payment reversal/cancellation is explicitly NOT proposed at this stage... marked here as a separate future capability, requiring its own design") — that separate future capability is what this section documents, realized. §22.9's own text is left unedited as the historical planning record, per this document's existing convention (see Section 19's own supersession of Section 17). Sales Payment's identical, still-unaddressed reversal gap (§21.4) remains explicitly out of scope and unchanged.
>
> This phase also bundled one small, adjacent, independently-discovered defect fix in `PurchaseOrdersService.cancel()` (§24.6) and left one newly-discovered, related defect in Goods Receipt reversal (Phase 3.7) explicitly un-fixed and deferred (§24.9), by explicit user decision.

### 24.1 Supplier Payment State Model

`SupplierPayment` gained a document-level lifecycle field, `status`, independent of the existing `accountingPostingStatus` field — the same two-field split already used by `GoodsReceiptStatus`/`GoodsReceiptPostingStatus` and `PurchaseReturnStatus`/`PurchaseReturnPostingStatus`:
...
enum SupplierPaymentPostingStatus {
  ...
  POSTED
  FAILED
  REVERSED   // new in Phase 3.8 — previously a three-value enum
}
```

`status` starts `ACTIVE` (schema default, safe for every pre-existing row — every payment recorded before this phase was, in fact, active). `REVERSED` is terminal — mirrors `PurchaseReturnStatus`'s own vocabulary choice ("REVERSED, never CANCELLED, for undoing a posted, effect-bearing document") rather than `PurchaseInvoiceStatus`'s `CANCELLED`.

New nullable columns: `reversalJournalEntryId`, `reversedAt`, `reversalReason` — same shape as `GoodsReceipt`/`PurchaseReturn`'s own reversal columns.

### 24.2 `reversePayment()` — ACTIVE → REVERSED

`PurchaseInvoicesService.reversePayment(actor, invoiceId, paymentId, dto)`:
- Undoes a single `ACTIVE` payment's effect on `PurchaseInvoice.amountPaid`/`paymentStatus`, and (post-commit, best-effort) its posted AP/Bank journal.
- Idempotent: re-invoking on an already-`REVERSED` payment is a no-op — checked twice, a soft pre-transaction read (so the common case never opens a transaction) and a hard re-check under lock (§24.5).
- No FIFO/consumption relationship between sibling payments on the same invoice (unlike GRNI matching) — any `ACTIVE` payment can be reversed independently of any other, in any order.
- Reversing a payment can drive a `CONFIRMED` invoice's `amountPaid` back to `0`, un-blocking `PurchaseInvoicesService.cancel()`'s existing `amountPaid.gt(0)` guard — `cancel()` itself is entirely unmodified by this phase.

### 24.3 Retry Endpoints

- `retryPaymentAccountingPosting()` — for a payment whose original posting attempt never succeeded (`accountingPostingStatus` `FAILED` or `NOT_POSTED`, e.g. a missing `paymentMethodId` at record time or accounting-service being unreachable). Rejected once `status === REVERSED`. No-op if already `POSTED`. Reuses the pre-existing `attemptPaymentPosting()`/`buildPaymentPostingRequest()` primitives unmodified.
- `retryPaymentAccountingReversal()` — for a `REVERSED` payment whose post-reversal accounting call (§24.2) failed and left `accountingPostingStatus` at `POSTED`. No-op if already `REVERSED`. 409s if there was never anything posted to reverse.
- Both mirror `PurchaseInvoicesService.retryAccountingPosting()`/`retryAccountingReversal()` (the invoice-level equivalents, Phase C1/C2) and `GoodsReceiptsService`'s own retry pair (Phase 3.7) exactly — same three-state contract (reject / no-op / attempt-and-surface-failure).

### 24.4 `amountPaid` / `paymentStatus` Invariants

`reversePayment()` decrements `PurchaseInvoice.amountPaid` by exactly the reversed payment's own amount, guarded against going negative (`ConflictException`, never clamped — mirrors every other negative-quantity/negative-value guard already established in this file, e.g. the `invoicedQuantity`/`matchedQuantity` guards). `paymentStatus` is recomputed as a genuine three-way branch — `UNPAID` (`amountPaid <= 0`) / `PARTIALLY_PAID` / `PAID` (`amountPaid >= total`) — the mirror image of `recordPayment()`'s own two-way accumulation logic (Section 22.9), since reversal is the only operation that can bring a paid invoice back to `UNPAID`.

### 24.5 Concurrency / Lock Order

`restoreAndReversePayment()` (the transactional core of `reversePayment()`) locks `purchase_invoices` (`FOR UPDATE`, single row) **then** `supplier_payments` (`FOR UPDATE`, single row) — the shortest lock chain in this module, since a payment has no children and (§24.2) no siblings requiring re-validation. Matches `recordPayment()`'s own parent-before-child order exactly, so the two can never deadlock against each other. The `ACTIVE`/`REVERSED` state is re-verified from the **locked** row, not the pre-transaction soft read — closing the race window where two concurrent `reversePayment()` calls for the same payment could otherwise both decrement `amountPaid`.

### 24.6 Purchase Order `invoicedQuantity` Cancellation Guard (adjacent fix)

`PurchaseOrdersService.cancel()` already rejected cancelling a `CONFIRMED` PO with any line `receivedQuantity > 0`. Phase 3.8 adds a second, independent guard in the same branch: reject if any line has `invoicedQuantity > 0`, even when `receivedQuantity === 0`.

This is reachable because a non-inventory-tracked PO line's `PurchaseInvoiceItem` never gets a `goodsReceiptItemId` match row, so `GoodsReceiptsService.reverse()`'s blocking guards (Phase 3.7) don't see it — a Goods Receipt covering an already-invoiced untracked line can be reversed, driving `receivedQuantity` back to `0` while `invoicedQuantity` stays positive. Without this guard, the PO could then be cancelled while a live `CONFIRMED` Purchase Invoice (and possibly a posted AP journal) still referenced it. The pre-existing `receivedQuantity` guard is untouched — this is a separate `if` block, not a modification of it.

### 24.7 RBAC / API Gateway / Angular

Three new routes, nested under the existing payments resource, mirroring the Goods Receipt (Section 19)/Purchase Return reverse+retry route shape exactly:

| Route | Permission |
|---|---|
| `POST /v1/purchase-invoices/:id/payments/:paymentId/retry-accounting-posting` | `purchase-invoices.record-payment` (existing — same permission `recordPayment()` itself uses) |
| `POST /v1/purchase-invoices/:id/payments/:paymentId/reverse` | `purchase-invoices.reverse-payment` (new) |
| `POST /v1/purchase-invoices/:id/payments/:paymentId/retry-accounting-reversal` | `purchase-invoices.reverse-payment` (new) |

New permission added to `libs/common/src/rbac/permissions.ts`, `apps/web/src/app/core/permissions/permissions.constants.ts`, and `apps/identity-service/prisma/seed.ts`'s `MANAGEMENT_PERMISSIONS` (auto-granted to the admin role via the existing seed loop, same mechanism Purchase Invoice V1's five permissions used).

Angular: the Purchase Invoices payment table gains Status (`ACTIVE`/`REVERSED`) and Accounting (`NOT_POSTED`/`POSTED`/`FAILED`/`REVERSED`) badge columns and three permission-gated, per-row action buttons (Reverse / Retry Posting / Retry Reversal), styled identically to the existing Purchase Return reverse-and-retry buttons — no new interaction pattern was introduced.

### 24.8 Accounting Journal Semantics

Original posting (`recordPayment()`, unchanged): `DEBIT ACCOUNTS_PAYABLE` / `CREDIT PAYMENT_METHOD`, `sourceType SUPPLIER_PAYMENT`. Reversal reuses accounting-service's existing generic `/reverse` endpoint unmodified: `sourceType SUPPLIER_PAYMENT` → `reversalSourceType SUPPLIER_PAYMENT_REVERSAL`, which swaps every line's debit/credit against the same account (`DEBIT PAYMENT_METHOD` / `CREDIT ACCOUNTS_PAYABLE`) and is idempotent by `(sourceService, reversalSourceType, sourceId)` — a repeated reversal call always returns the first reversal entry, never creating a duplicate. Live-verified (journal entries inspected directly in `accounting_db`) during this phase's implementation and again at final pre-commit audit.

### 24.9 Deferred / Known Limitations — explicit, not fixed in this phase

- **Inventory valuation (negative `Stock.totalValue`)**: `StockReturnsService`'s subtractive branch (shared by `PURCHASE_RETURN` and `PURCHASE_REVERSAL`, Phase 3.5/3.7) can drive `Stock.totalValue` negative when the weighted average has drifted since the original movement being returned/reversed. Identified during this phase's own audit work, **explicitly left untouched** — `apps/inventory-service/**` (including `stock-returns.service.ts`) was not modified anywhere in Phase 3.8. Remains an open design decision (clamp-at-zero vs. reject vs. accept-as-known-limitation) for a future phase, not decided here.
- **Goods Receipt reversal / untracked-line gap**: see §24.6 — `GoodsReceiptsService.reverse()`'s blocking guards don't check `invoicedQuantity` for non-inventory-tracked lines. Phase 3.8's PO cancel guard (§24.6) contains the one concrete downstream consequence (an uncancellable-state PO becoming cancellable); the root cause in Goods Receipt reversal itself was explicitly deferred to a future phase, by explicit user decision, rather than folded into this phase's scope.
- **Sales Payment**: has the identical no-reversal gap (§21.4) — untouched, out of scope, a separately-scoped Sales-module decision if ever addressed.
- **`NOT_POSTED` retry-posting**: `attemptPaymentPosting()` always transitions a payment away from `NOT_POSTED` synchronously inside `recordPayment()`'s own post-commit step, so this state cannot persist under normal operation — it is only reachable via a genuine crash between commit and that attempt. Covered by a dedicated unit test and, for the live/Docker verification pass, by a direct one-row database state simulation of that crash window (not a code change) to prove the real endpoint's handling end-to-end.

### 24.10 Test / Verification Summary

- `purchase-invoices.service.spec.ts`: 24 new `it()` blocks (the "Phase 3.8 — Supplier Payment reversal" describe block), including a dedicated soft-check-vs-hard-lock race test and an explicit `NOT_POSTED`-starting retry-posting test.
- `purchase-orders.service.spec.ts`: 2 new tests for the `invoicedQuantity` guard, plus regression coverage confirming the pre-existing `receivedQuantity` guard is unaffected.
- `purchase.guards.spec.ts` (API Gateway): 3 new RBAC probe-controller tests (deny + allow for all three new routes).
- Full backend regression (`apps/accounting-service`, `apps/api-gateway`, `apps/identity-service`, `apps/inventory-service`, `apps/master-data-service`, `apps/purchase-service`, `apps/sales-service`, `libs`): 946/946 passing at final audit.
- Angular: `tsc --noEmit` clean, `ng build --configuration production` clean, Karma 39/39.
- Live/Docker: fresh end-to-end verification performed twice (Stage 2 implementation and the final pre-commit audit), covering successful-payment reversal with journal inspection, `FAILED`-payment reversal without an accounting call, multi-payment partial reversal, `PAID → UNPAID` → invoice cancellation, reversal idempotency (no duplicate journal), accounting-service outage/restart for both retry-posting and retry-reversal, the PO `invoicedQuantity` guard, and RBAC deny/allow through the gateway.
...
### 2026-09-25 — Supplier Payment reversal and retry lifecycle implemented (Phase 3.8; backend, gateway, Angular)
- **Phase/feature**: Full implementation of Supplier Payment reversal/retry — the "separate future capability" §22.9 explicitly deferred when Supplier Payment V1 (Phase B) was first implemented. Undertaken after a dedicated read-only audit of the whole Purchase/Inventory/Accounting lifecycle identified Supplier Payment as the one remaining posted document type with no reversal path anywhere in the module (every other posted document — Goods Receipt, Purchase Invoice, Purchase Return — already had one). Bundled one small, independently-discovered adjacent defect fix in `PurchaseOrdersService.cancel()` (see below).
- **What was changed**:
  - `SupplierPayment` gained a document-level `status` field (`SupplierPaymentStatus`: `ACTIVE`/`REVERSED`, default `ACTIVE`) independent of the existing `accountingPostingStatus`; `SupplierPaymentPostingStatus` gained a fourth value, `REVERSED`; new nullable `reversalJournalEntryId`/`reversedAt`/`reversalReason` columns, same shape as `GoodsReceipt`/`PurchaseReturn`'s own reversal columns.
  - Three new `PurchaseInvoicesService` methods: `reversePayment()` (`ACTIVE → REVERSED`, decrements `PurchaseInvoice.amountPaid`, recomputes `paymentStatus`, reverses the posted AP/Bank journal post-commit best-effort), `retryPaymentAccountingPosting()` (recovers a `FAILED`/`NOT_POSTED` original posting), `retryPaymentAccountingReversal()` (recovers a failed post-reversal accounting attempt) — full detail in Section 24.
  - One adjacent, independently-scoped fix: `PurchaseOrdersService.cancel()` gained a second guard rejecting cancellation when any line has `invoicedQuantity > 0`, alongside its existing `receivedQuantity > 0` guard (§24.6) — discovered via live reproduction during this phase's own Docker verification, not merely theorized.
  - API Gateway: three new forwarding routes, new `purchase-invoices.reverse-payment` RBAC permission (retry-posting reuses the existing `purchase-invoices.record-payment`).
  - Angular: Purchase Invoices payment table gained Status/Accounting badge columns and three permission-gated per-row actions, mirroring the existing Purchase Return button conventions.
- **Why**: The user commissioned a read-only audit of the full Purchase/Inventory/Accounting lifecycle to find the next required phase after Goods Receipt reversal (Phase 3.7); Supplier Payment's total absence of a reversal path — the last posted Purchase document type without one — was identified and, after an explicit multi-round design review (state transitions, retry-vs-reversal semantics, lock order, amountPaid invariants, accounting failure/retry behavior, idempotency, RBAC), approved for implementation as Phase 3.8. Implementation proceeded in two explicitly-approved stages (Stage 1: schema/migration/purchase-service backend; Stage 2: gateway/RBAC/frontend), each independently verified — including two full, independent pre-commit re-audits reading the actual committed code fresh rather than trusting prior summaries — before commit.
- **Files modified**: `apps/purchase-service/prisma/schema.prisma`; new migration `prisma/migrations/20260925070448_supplier_payment_reversal_v1/`; `src/purchase-invoices/purchase-invoices.service.ts`, `purchase-invoices.controller.ts`, `dto/supplier-payment-response.ts`, new `dto/reverse-supplier-payment.dto.ts`; `src/purchase-orders/purchase-orders.service.ts` (adjacent guard only); `src/purchase-invoices/purchase-invoices.service.spec.ts`, `src/purchase-orders/purchase-orders.service.spec.ts`. API Gateway: `src/purchase/purchase-invoices.controller.ts`, `dto/purchase-invoice.dto.ts`, `purchase.guards.spec.ts`. RBAC: `libs/common/src/rbac/permissions.ts`, `apps/web/src/app/core/permissions/permissions.constants.ts`, `apps/identity-service/prisma/seed.ts`. Angular: `src/app/features/purchase/purchase-invoices/purchase-invoice.service.ts`, `purchase-invoice-list.component.ts`, `.html`; `src/app/features/purchase/models/purchase.models.ts`. 19 files total (17 modified + 2 new).
- **Database/schema/migration changes**: New migration `20260925070448_supplier_payment_reversal_v1` — `ALTER TYPE "SupplierPaymentPostingStatus" ADD VALUE 'REVERSED'`, `CREATE TYPE "SupplierPaymentStatus"`, `ALTER TABLE supplier_payments ADD COLUMN` x4 (one `NOT NULL DEFAULT 'ACTIVE'`, three nullable) — purely additive, no prior migration edited, applied via `prisma migrate deploy` against local `purchase_db`; `prisma migrate status` confirmed clean both immediately after applying and again at final audit.
- **API changes**: New `POST /v1/purchase-invoices/:id/payments/:paymentId/{retry-accounting-posting,reverse,retry-accounting-reversal}`, gated by `purchase-invoices.record-payment` (retry-posting) and the new `purchase-invoices.reverse-payment` (reverse, retry-reversal).
- **Frontend changes**: Purchase Invoices detail view's payment table gains Status/Accounting badges and Reverse/Retry Posting/Retry Reversal buttons, permission-gated, mirroring existing Purchase Return button conventions exactly.
- **Tests added/changed**: 24 new cases in `purchase-invoices.service.spec.ts`'s Phase 3.8 describe block (including a dedicated lock-race test and an explicit `NOT_POSTED` retry-posting test added after initial review), 2 new + regression coverage in `purchase-orders.service.spec.ts`, 3 new RBAC probe tests in `purchase.guards.spec.ts`.
- **Verification results**: `npx jest` across all seven backend services + `libs` → **946/946 passed** (re-run fresh at final audit, not merely reported from an earlier run). `tsc --noEmit` clean for `purchase-service`/`api-gateway`/`identity-service`/`apps/web`. `ng build --configuration production` → clean. Karma → 39/39. Live Docker verification performed twice independently (Stage 2, and again at final pre-commit audit with entirely fresh data): successful payment → reverse → accounting reversal (journal entries inspected directly in `accounting_db`, confirmed exact debit/credit swap and correct `reversesJournalEntryId` linkage); `FAILED` payment → reverse with zero accounting calls (confirmed via `journal_entries` row count); two payments → reverse the earlier, later stays `ACTIVE`; sole payment on a `PAID` invoice → reverse → `UNPAID` → invoice cancellation unblocked; double reversal → idempotent, no duplicate journal; accounting-service stop/restart → both `retry-accounting-posting` and `retry-accounting-reversal` recover correctly; PO `invoicedQuantity` guard → 409 blocked, live-reproduced via the actual untracked-line/GR-reversal path; RBAC → permission revoke/re-grant correctly denies/allows through the gateway.
- **Architectural decisions**: Method names (`retryPaymentAccountingPosting`/`retryPaymentAccountingReversal`, not the shorter `retryAccountingPosting`/`retryAccountingReversal` used at the invoice level) were required to avoid colliding with `PurchaseInvoicesService`'s own existing same-named invoice-level methods — cosmetic, not a design deviation. `reversePayment()`'s lock order (`purchase_invoices → supplier_payments`) and idempotency pattern (soft pre-check + hard lock-protected re-check) directly reuse the conventions already established by `GoodsReceiptsService.reverse()` (Phase 3.7) and `PurchaseReturnsService.reverse()` (Phase 3.6) — no new architecture was invented. The negative-inventory-valuation issue discovered during this phase's own audit work (§24.9) was deliberately **not** fixed — `apps/inventory-service/**` was confirmed untouched via `git diff --stat` at every verification pass. The Goods Receipt reversal / untracked-line gap (§24.6, §24.9) was also deliberately **not** fixed in Goods Receipt itself — only its one concrete downstream consequence (an uncancellable PO state) was guarded against in Purchase Order cancellation.
- **Known limitations/technical debt**: See §24.9 in full — inventory valuation negative-value issue (deferred), Goods Receipt reversal's untracked-line gap (deferred, root cause), Sales Payment's identical no-reversal gap (out of scope), `NOT_POSTED` retry-posting's practical unreachability under normal operation (covered by a database-state-simulated live test, not a code path gap).
- **Anything deliberately NOT changed**: `apps/inventory-service/**` (including `stock-returns.service.ts`), `apps/sales-service/**`, `apps/accounting-service/**`, `apps/purchase-service/src/goods-receipts/**`, `apps/purchase-service/src/purchase-returns/**` — confirmed untouched via `git diff --stat` against `4a3405220fd57ffe100e9d21b81143dd888e05af` at every verification pass, including the final independent pre-commit audit. `PurchaseInvoicesService.cancel()` — confirmed unmodified (reversal only makes its existing `amountPaid.gt(0)` guard newly satisfiable, never changes the guard itself). `PurchaseOrdersService.cancel()`'s pre-existing `receivedQuantity` guard — confirmed unmodified, the new `invoicedQuantity` guard is a separate, independent `if` block.
- **Commit hash**: `4acc47f289ab56da4b93833aa415c71ed5d94b23`
