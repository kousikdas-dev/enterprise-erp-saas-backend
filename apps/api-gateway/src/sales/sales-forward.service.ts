...
interface SalesEnvelope<T> {
  success?: boolean;
  data?: T;
}

export interface SalesForwardOptions {
...
@Injectable()
export class SalesForwardService {
  private readonly logger = new Logger(SalesForwardService.name);
  ...
  async forward<T>(options: SalesForwardOptions): Promise<T> {
    const url = `${this.salesBaseUrl()}${options.path}`;
    const body = this.stripTenant(options.body);
    const params = this.stripTenant(options.query);
    try {
      const response = await firstValueFrom(
        this.http.request<SalesEnvelope<T>>({
  ...
  private salesBaseUrl(): string {
    return this.downstream.getUrl('sales').replace(/\/$/, '');
  }
  ...
  private rethrowUpstream(error: unknown): never {
    ...
    this.logger.warn('Sales proxy failed');
    throw new BadGatewayException('Sales service error');
  }
